...
class PolynomialInputDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.submit_button = QPushButton("Построить модель регрессии по системе функции", self)
...
class InputArrayDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.x_input_field.setPlaceholderText("Пример: 0.1,0.2,0.3,0.4")
        self.x_input_field.setValidator(QDoubleValidator(0.0, 1.0, 10))
        ...
        self.y_input_field.setValidator(QDoubleValidator(0.0, 1.0, 10))
    ...
    def save_and_display(self):
        ...
        if x_text and y_text:
            try:
                x_values = np.array(list(map(float, x_text.split(','))))
                y_values = np.array(list(map(float, y_text.split(','))))
                if len(x_values) != len(y_values):
                    QMessageBox.critical(self, "Ошибка ввода", "Количество введенных значений x и y должно быть одинаковым.")
                    return
                ...
                QMessageBox.critical(self, "Ошибка ввода", "Неверный формат ввода. Пример: 0.1,0.2,0.3,0.4")

    def display_array(self):
         if self.array is not None:
              self.parent().display_data_on_figure2(self.array, "Данные, которые ввел пользователь")  # вызываем метод родительского окна
         else:
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.regression_button = QPushButton('Построить регрессию')
        ...
        self.extra_button2.clicked.connect(self.check_multicollinearity_dialog)  # Подключаем функцию проверки на мультиколлинеарность
    ...
    def generate_2d_array(self):
        ...
        def on_save_clicked():
            ...
            if array is None:
                 ...
                 QMessageBox.information(self, "Сохранение массива", f"Массив, сгенерированный по распределению: {selected_distribution} сохранен в {save_path}.")
                 self.data_for_figure1 = array
                 self.display_data_on_figure1(self.data_for_figure1, f"Сгенерированные данные по распределению {selected_distribution}")
            dialog.accept()
    ...
    def build_confidence_interval_dialog(self):
         ...
         button1 = QPushButton("Для 1 графика", dialog)
         button2 = QPushButton("Для 2 графика", dialog)
         ...
         def build_for_figure1():
                ...
                try:
                   ...
                   lower_bound, upper_bound = MathFunctions.confidence_interval(pd.DataFrame({'x': x, 'y': y}), target_column)
    ...
    def check_multicollinearity_dialog(self):
         ...
         def check_for_figure1():
             ...
             try:
                ...
                y = self.data_for_figure1[:, 1]
                target_column = "y"
                multicollinear_features = MathFunctions.check_multicollinearity(pd.DataFrame({'x': x, 'y': y}), target_column)
            ...
            try:
                if self.polynomial is None: #Выполнение линейной регрессии, если многочлен не задан
                    ...
                    if figure_num == 1:
                        ...
                        r_squared = MathFunctions.calculate_r_squared(pd.DataFrame({'x': x_values, 'y': y}), 'y', regression_line)
                        QMessageBox.information(self, "Результат регрессии",f"Регрессия по многочлену, R^2: {r_squared}")
                    ...
                    if figure_num == 2:
                        ...
                        regression_line = np.array([func(val) for val in x_values])
                        self.figure2.plot_polynomial_regression(x_values, self.dataframe[target_column].values, regression_line)
                        r_squared = MathFunctions.calculate_r_squared(self.dataframe, target_column, regression_line)
                        QMessageBox.information(self, "Результат регрессии",f"Регрессия по многочлену, R^2: {r_squared}")
    ...
    def calculate_r_squared(self, figure_num):
        ...
        try:
            if figure_num == 1:
                ...
                regression_line = np.array([linear_func(x_val) for x_val in x])
                r_squared_value = MathFunctions.calculate_r_squared(pd.DataFrame({'x': x, 'y': y}), target_column, regression_line)
